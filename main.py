...
x_train_np, x_test_np, y_train_np, y_test_np = train_test_split(x, y, test_size=0.30, random_state=742, shuffle=True)
...
columns = [column for column in x.columns] + ['admit']

with open('./datasets/processed/law/law_processed_train.csv', 'w', newline='') as f:
...
with open('./datasets/processed/law/law_processed_test.csv', 'w', newline='') as f:
    # using csv.writer method from CSV package
    write = csv.writer(f)
...
with open('D:\\Facultate\Disertatie\\experiments\\fair-ai\\datasets\\processed\\law\\law_processed_train.csv', newline='') as f:
    reader = csv.reader(f)
    train = list(reader)
    with open('D:\\Facultate\Disertatie\\experiments\\fair-ai\\datasets\\processed\\law\\law_processed_test.csv',
              newline='') as f:
        reader = csv.reader(f)
        test = list(reader)

...
import copy
from algorithms.fairness_measures import demographic_parity_urm_vs_others
...
class DecisionTree:
    ...
    def build_tree(self):
        ...
        if score_measure == 'ig':
            ...
            current_score = demographic_parity_urm_vs_others(self.train, -1)
    ...
    def calculate_demographic_parity_urm_vs_others(self, clusters):
        weights = []
        for i in range(0, len(clusters)):
            weights.append(len(clusters[i]))
        total_number_instances = sum(weights)
        ...
        for i in range(0, len(clusters)):
            demographic_parity = demographic_parity + weights[i] * demographic_parity_urm_vs_others(clusters[i], -1)
    ...
    def select_best_feature(self, dataset, score_measure, current_score, variable_importance, features_already_used):
        ...
        for index in features:  # for each feature
            ...
            if len(possible_values) == 2 and ((possible_values[0] == 0 and possible_values[1] == 1) or (possible_values[0] == 1 and possible_values[1] == 0)):
                ...
                if score_measure == 'ig':
                    ...
                    demographic_parity = self.calculate_demographic_parity_urm_vs_others(clusters)
        ...
        if feature_index == 999:
            return {'index': None, 'feature': None, 'value': None, 'score': feature_score,
